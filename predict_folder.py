"""
Script for running disparity prediction using the ACVNet model.

This script loads the configuration from the 'config.json' file.

It runs the prediction and displays the result
in a window using matplotlib. Processing will pause and wait
for the window to be closed before continuing to the next image.

Required inputs in 'config.json':
-----------------------------------
- checkpoint_pth: Path to the trained model file (.pth).
- paths:
    - left_folder: Path to the folder with left images.
    - right_folder: Path to the folder with right images.
    - disp_folder: Path to the folder with ground-truth disparity maps.
